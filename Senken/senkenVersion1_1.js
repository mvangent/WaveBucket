...
function drawLFOI() {
    ...
    drawLFOIVisual = requestAnimationFrame(drawLFOI);
    lfoIanalyser.getByteTimeDomainData(dataArrayLFOI);

    //set canvas background
    canvasLFOICtx.fillStyle = 'rgb(150, 100, 150)';
    canvasLFOICtx.fillRect(0, 0, WIDTH_LFO_I, HEIGHT_LFO_I);
    // set canvas linewidth and stroke
    canvasLFOICtx.lineWidth = 2;
    canvasLFOICtx.strokeStyle = 'rgb(205, 205, 200)';
    // begin drawing a path
...
function activateLFO(lfoFreq, scale) {
    ...
    lfo.connect(lfoIanalyser);

    lfoIanalyser.connect(gain);
...
LFOLoadButtonElement.addEventListener("click", function () { activateLFO(getLFOFrequency(), getLFOScale()) });
